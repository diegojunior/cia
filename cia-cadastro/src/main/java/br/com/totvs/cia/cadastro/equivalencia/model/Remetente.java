package br.com.totvs.cia.cadastro.equivalencia.model;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.Id;
import javax.persistence.Table;

import org.hibernate.annotations.GenericGenerator;

import br.com.totvs.cia.cadastro.equivalencia.json.RemetenteJson;
import br.com.totvs.cia.infra.model.Model;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.NoArgsConstructor;

@Data
@Entity
@NoArgsConstructor
@AllArgsConstructor
@Table(name = "CD_REMETENTE")
@EqualsAndHashCode(callSuper = false, of = {"idLegado"})
public class Remetente implements Model{
	
	private static final long serialVersionUID = 1L;

	@Id
	@Column(name = "ID")
	@GeneratedValue(generator = "system-remetente-uuid")
	@GenericGenerator(name = "system-remetente-uuid", strategy = "uuid")
	private String id;
	
	@Column(name = "ID_LEGADO")
	private String idLegado;
	
	@Column(name = "CODIGO")
	private String codigo;
	
	public Remetente(final RemetenteJson json) {
		this.id = json.getId();
		this.idLegado = json.getIdLegado();
		this.codigo = json.getCodigo();
	}
}