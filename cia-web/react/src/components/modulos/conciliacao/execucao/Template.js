...
export default class Template extends CrudComponent {
	...
	consultar (data, perfil, status) {
		
		ConciliacaoService
			.search(data, perfil, status)
				.then (data => {
					if (data.length == 0) {
						this.infoMessage('Nenhuma Conciliação Encontrada!');
					}
					this.setState({
						list: data
					});
					...
					<div className="panel-body">
						<div className="dataTable_wrapper">
							...
							<Datatable
								...
								metaData={["data", "perfil.identificacao.codigo", "status.nome"]}
