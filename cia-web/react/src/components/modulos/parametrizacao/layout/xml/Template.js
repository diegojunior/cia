...
export default class Template extends CrudComponent {
	...
	consultar (codigo, descricao, status) {
		
		LayoutXmlService.getBy(codigo, descricao, status)
			.then (data => {
					if (data.length == 0) {
						this.infoMessage('Nenhum Layout Xml Encontrado!');
					}
					this.setState({
						list: data
					});
	...
	remover () {
		return (event) => {
			...
			this.confirmMessage('Exclusão', 'Deseja remover o(s) registro(s) selecionado(s)?', () => {
				LayoutXmlService.remover(this.state.selectedList)
					.then (data => {
				...
				<div style={displayDatatable} className="panel panel-listagem" id="listPanel">
					...
					<div className="panel-body">
						<div className="dataTable_wrapper">
							...
							<Datatable id="datatableLayoutXml"
