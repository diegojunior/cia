...
export default class AddRemoveMultiplesInputs extends ValidatorComponent {
	...
	removeLocalRow() {
		...
		for (var index in this.props.fieldsConfig) {
			...
			if(fieldConfig.required) {
				...
				this.removeValidationErrors(this.state, fieldConfig.key+lastIndex, 'required');
			}
			if(fieldConfig.integerOnly) {
				this.removeFieldValidation(this.state, fieldConfig.key+lastIndex, 'integerOnly');
				this.removeValidationErrors(this.state, fieldConfig.key+lastIndex, 'integerOnly');
			}
		}
	...
	removeRow(stateParam) {
		...
		if (stateParam.rows) {
			var lastIndex = stateParam.rows.length ? stateParam.rows.length - 1 : 0;
			for (var index in this.props.fieldsConfig) {
				var fieldConfig = this.props.fieldsConfig[index];
				if(fieldConfig.required) {
	...
	fieldText (fieldConfig, index) {
		...
		if (this.state.rows[index][fieldConfig.key+index] && 
			this.state.rows[index][fieldConfig.key+index].readOnly) {
