package br.com.totvs.cia.gateway.amplis.processamento.posicao.rendavariavel.emprestimo.converter;
...
@NoArgsConstructor
public class PosicaoAmplisRendaVariavelEmprestimoConverter extends JsonGatewayConverter<PosicaoEmprestimoJson, UnidadeProcessamentoJson> {
	
	private String dataPosicao;
	
	public PosicaoAmplisRendaVariavelEmprestimoConverter(final String dataPosicao) {
		this.dataPosicao = dataPosicao;
	}
	
	@Override
	public UnidadeProcessamentoJson convertFrom(final PosicaoEmprestimoJson posicao) {
		try {
			UnidadeProcessamentoJson unidade = new UnidadeProcessamentoJson(ServicoEnum.RENDAVARIAVEL_EMPRESTIMO, this.dataPosicao, posicao.getCarteira() != null ? posicao.getCarteira().getCodigo() : "");
			...
			unidade.add(new CampoProcessamentoJson("contrato", posicao.getContrato().getCodigo()));
			...
			unidade.add(new CampoProcessamentoJson("vlEmprestimo", replaceDotToCommaAndFormat(posicao.getVlEmprestimo())));
			unidade.add(new CampoProcessamentoJson("vlLiquido", replaceDotToCommaAndFormat(posicao.getVlLiquido())));
			
			return unidade;
		} catch (Exception ex) {
			throw new ConverterException("Ocorreu erro ao converter posição de Emprestimo de Renda Variavel para Unidade de Processamento Json.", ex);
		}
	}
}